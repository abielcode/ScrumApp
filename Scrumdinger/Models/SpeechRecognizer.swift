...
extension SpeechRecognizer {
    func transcribe() {
        DispatchQueue(label: "scrumdinger.speechrecognizer", qos: .background).async { [weak self] in
...
extension AVAudioSession {
    func hasPermissionToRecord() async -> Bool {
        await withCheckedContinuation({ continuation in
            requestRecordPermission { authorizedStatus in
                continuation.resume(returning: authorizedStatus)
            }
        })
    }
}
